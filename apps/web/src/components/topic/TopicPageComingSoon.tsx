  ...
  return (
    <article className="max-w-2xl">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm mb-8">
        <span className={`text-xs px-2 py-0.5 rounded border font-medium ${DIFFICULTY_COLORS[topic.difficulty]}`}>
          {topic.difficulty}
        </span>
      ...
      {prereqTopics.length > 0 && (
        <div className="mb-6">
          ...
          <ul className="space-y-1">
            {prereqTopics.map((t) => (
              <li key={t.id}>
                <Link href={`/topics/${t.slug}`} className="text-sm text-brand-text hover:underline underline-offset-2">
                  {t.title}
                </Link>
              </li>
            ))}
      ...
      {relatedTopics.length > 0 && (
        <div className="mb-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-ink-muted mb-2">Related topics</p>
          <ul className="space-y-1">
            {relatedTopics.map((t) => (
              <li key={t.id}>
                <Link href={`/topics/${t.slug}`} className="text-sm text-brand-text hover:underline underline-offset-2">
